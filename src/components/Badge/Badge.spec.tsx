...
describe('Badge', () => {
  ...
  describe('Priorities', () => {
    it('should render default badge', async () => {
      const driver = createDriver(<Badge priority={BADGE_PRIORITY.default}>I'm default badge</Badge>);
      expect(await driver.isDefault()).toBeTruthy();
      expect(await driver.exists()).toBe(true);
    });
    it('should render light badge', async () => {
      const driver = createDriver(<Badge priority={BADGE_PRIORITY.light}>I'm light badge</Badge>);
      expect(await driver.isLight()).toBeTruthy();
      expect(await driver.exists()).toBe(true);
    });
    it('should render primary badge', async () => {
      const driver = createDriver(<Badge priority={BADGE_PRIORITY.primary}>I'm primary badge</Badge>);
